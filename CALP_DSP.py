...
def k_weighting_sos(fs):
    ...
    shelf = high_shelf_sos(
        fs, f0=K_WEIGHT_SHELF_HZ, gain_db=K_WEIGHT_SHELF_GAIN_DB, slope=K_WEIGHT_SHELF_SLOPE
    )
...
def collect_frame_metrics(
    ...
    if not frame_metrics:  # fallback – should never happen
        seg = slice(0, n)
        frame_metrics.append(band_features(mono_w[seg]))
        band_metrics["low"].append(band_features(low_sig[seg]))
        band_metrics["mid"].append(band_features(mid_sig[seg]))
...
# CONTROL
# ============================================================
def build_gain_curves(analysis: dict, target_lufs: float):
    ...
    global_frames = attack_release_smooth(
        global_frames, attack=GLOBAL_ATTACK, release=GLOBAL_RELEASE
    )

    for k in band_frames:
        band_frames[k] = attack_release_smooth(
            band_frames[k], attack=BAND_ATTACK[k], release=BAND_RELEASE[k]
        )
...
def true_peak_limiter(x, sr, ceiling=LIMITER_CEILING,
...
def process(input_path: str, output_path: str,
    ...
    filters = get_filters(sr)
    low_f, mid_f, high_f, kw = (
        filters["low"], filters["mid"], filters["high"], filters["kw"]
    )
    ...
    global_gain = lin(sample_hold(global_frames, len(audio), hop, frame_len)).astype(np.float32)
    low_gain = lin(sample_hold(band_frames["low"],  len(audio), hop, frame_len)).astype(np.float32)
    mid_gain = lin(sample_hold(band_frames["mid"],  len(audio), hop, frame_len)).astype(np.float32)
    high_gain = lin(sample_hold(band_frames["high"], len(audio), hop, frame_len)).astype(np.float32)
